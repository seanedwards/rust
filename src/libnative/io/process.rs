...
fn waitpid(pid: pid_t) -> p::ProcessExit {
    ...
    fn waitpid_os(pid: pid_t) -> p::ProcessExit {
        ...
        #[cfg(target_os = "android")]
        mod imp {
            pub fn WIFEXITED(status: i32) -> bool { (status & 0xff) == 0i32 }
            pub fn WEXITSTATUS(status: i32) -> i32 { (status >> 8i32) & 0xff }
            pub fn WTERMSIG(status: i32) -> i32 { (status >> 8i32) & 0x7f }
        }

        #[cfg(target_os = "macos")]
        #[cfg(target_os = "freebsd")]
        mod imp {
            pub fn WIFEXITED(status: i32) -> bool { (status & 0x7f) == 0i32 }
            pub fn WEXITSTATUS(status: i32) -> i32 { status >> 8i32 }
        ...
        return if imp::WIFEXITED(status) {
